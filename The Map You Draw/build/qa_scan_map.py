"""QA scanner for The Map You Draw (PyMuPDF).
...
2. Zone integrity: on atlas pages the big dashed canvas box must contain no
   words at all — marginalia bleeding into the drawing zone is this
   template's failure mode (mirrors the DRAW-box check from TWIW/Wander).
...
PDF = "../the-map-you-draw_v1.0_letter.pdf"
...
for pno in range(doc.page_count):
    ...
    is_atlas_page = any(w[4] == "MARK" for w in words) and \
        any(w[4] == "PERSONAL" for w in words)
    if is_atlas_page:
        boxes = [d["rect"] for d in page.get_drawings()
                 if d["rect"].width > 400 and d["rect"].height > 200]
        if not boxes:
            problems.append(f"p{pno+1}: atlas page has no canvas box drawing")
        else:
            ...
            for x0, y0, x1, y1, w, *_ in words:
                if fitz.Rect(x0, y0, x1, y1).intersects(inner):
                    problems.append(f"p{pno+1}: word {w!r} inside canvas box")
...
if problems:
    ...
    for p in problems:
        print(" -", p)
    sys.exit(1)
print("QA clean: no overflow, no words in canvas boxes, Tc always reset.")
